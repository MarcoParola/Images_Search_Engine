...
\end{verbatim}
The final training set is composed by a total of 7786 images, while the validation set has a total of 856 images. \\ 
...
\vspace{10mm}
The following figures show the distribution of the classes of the validation set, after the execution of the snippet of code shown above :\\
...
k = 10 &  mAP euclidean & avg dist & mAP cosine & avg sim & $\#$Items\\
\hline
\textbf{g=3 h=2} & \textbf{0.772} & \textbf{22.772} & \textbf{0.801} & \textbf{0.619}  & \textbf{855} \\
g=3 h=3 & 0.631 & 27.532 & 0.663 & 0.490   & 849 \\
g=3 h=4 & 0.357 & 29.709 & 0.404 & 0.409   & 711 \\ 
\hline
\textbf{g=4 h=2} & \textbf{0.793} & \textbf{22.285} & \textbf{0.814} & \textbf{0.634}  & \textbf{856} \\
g=4 h=3 & 0.608 & 27.865 & 0.651 & 0.474   & 843 \\
g=4 h=4 & 0.414 & 29.658 & 0.446 & 0.411   & 736 \\
\hline
\textbf{g=5 h=2} & \textbf{0.780} & \textbf{22.456} & \textbf{0.807} & \textbf{0.627}  & \textbf{855} \\
g=5 h=3 & 0.691 & 26.175 & 0.728 & 0.527   & 843 \\
g=5 h=4 & 0.486 & 28.903 & 0.509 & 0.432   & 741 \\
\hline
\textbf{g=6 h=2} & \textbf{0.794} & \textbf{21.995} & \textbf{0.822} & \textbf{0.639}  & \textbf{855} \\
g=6 h=3 & 0.677 & 26.748 & 0.705 & 0.513   & 843 \\
g=6 h=4 & 0.524 & 27.908 & 0.545 & 0.457   & 736 \\
\hline
\textbf{g=7 h=2} & \textbf{0.795} & \textbf{22.128} & \textbf{0.820} & \textbf{0.639}   & \textbf{854} \\
g=7 h=3 & 0.584 & 28.075 & 0.629 & 0.470   & 835 \\
g=7 h=4 & 0.304 & 29.794 & 0.323 & 0.407   & 696 \\
\hline
\textbf{g=8 h=2} & \textbf{0.789} & \textbf{22.137} & \textbf{0.818} & \textbf{0.635}   & \textbf{856} \\
g=8 h=3 & 0.566 & 27.591 & 0.615 & 0.483   & 835 \\
g=8 h=4 & 0.380 & 29.613 & 0.405 & 0.414   & 717 \\
\hline
\end{tabular}
\vspace{2mm}
\end{center}
We performed different tests varying the value of \textit{w}; in the previous table the values shown are calculated with w=8, indeed from other tests we noted a small decrease on the value of mAP, but not so relevant, so we reported only the best ones.\\

\subsection{Conclusion of Features Extraction approach}
Observing the results obtained from the experiments based on features extraction approach, we can say that the distances between the features of the objects extracted with Resnet50 are greater than that obtained using Resnet50v2. \\
...
It is interesting to note that given a certain value of \textit{g} and varying the value of \textit{h}, the best results are given for h=3, because for greater value the mAP always decrease, while for smaller ones (h=2) in some cases the mAP decrease, in others increase, but we also observe a very significant increase in the number of the items to sort.
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[scale=0.83]{./img/resnet50_cosine.png}
    \end{minipage}
\vspace{2mm}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[scale=0.62]{./img/resnet50_visitedObj.png}
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[scale=0.62]{./img/resnet50_map.png}
    \end{minipage}
\end{figure}
\vspace{3mm}
...
\hline
  Model & Hyperparameters & \textbf{mAP} euclidean & avg dist & \textbf{mAP} cosine & avg sim & Items\\
\hline
...
During this part of the project we analyzed space of the hyperparameter values after running different tests in the previous sections.\\
All the following figure show some plot where the data are computed using the cosine similarity, but similar results are obtained using the euclidean distance, we didn't report them to avoid redundancy.\\

The first aspect that we noted is that we obtained the highest values of the mAP setting low value of the number of hyperplane \textit{h} (h=2) fixed the length of the segment \textit{w} and the number of g-function \textit{g}, indeed increasing the value of h we can observe lower curves. \\
Moreover we can observe that increasing the value of \textit{w} also the value of mAP increases.
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[scale=0.23]{./img/lineChart_g4.png}
    \end{minipage}
\vspace{2mm}
\end{figure}


The following two figures show the variation of the mAP value and the number of visited objects when we increase the number of g-function.
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[scale=0.19]{./img/lineChart_h2.png}
    \end{minipage}\hfill
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[scale=0.19]{./img/lineChart_h2_objRetrieved.png}
    \end{minipage}
\end{figure}
\vspace{3mm}


The following figure shows the decrease of the mAP value when we increase the number of hyperplanes for each g-function.
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[scale=0.23]{./img/lineChart_w4.png}
    \end{minipage}
\vspace{2mm}
\end{figure}
