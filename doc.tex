...
\begin{titlepage}
\begin{center}
{\Huge \textbf{Multimedia Information Retrieval \\ and Computer Vision} \par}
\vspace{18mm}
{\Large Project developed by \par}
\vspace{3mm}
...
\begin{figure}[h]
\centering
\includegraphics[width=.3\textwidth]{img/unipi}
\end{figure}
\vspace{9mm}
...
The Web Search Engine is based on the \textbf{Locality Sensitive Hashing} (LSH) index and on the pre-trained convolutional neural network ResNet50 with weights computed on the "Imagenet" dataset.
...
\end{tabular}
\end{center}
We tested the LSH index also with different values of \textit{w}, but we concluded that w=4 is the best values with Resnet50, because using lower values the buckets are sparse, while increasing \textit{w} the mAP remains constant.
\vspace{2mm}
...
This is already anticipated in the analisys of \textit{section 5.1}.
...
We computed this phase adopting a \textbf{trial and error approach}, trying different hyperparameter values for the fully connected neural network appended to the pretrained convolutional one and we compared the performance.\\
...
\textbf{Model 4} & \textbf{0.8808} & \textbf{0.3772} &  \textbf{0.893} &  \textbf{8.997} & \textbf{0.908} & \textbf{0.729} & \textbf{32791}\\
\hline
\end{tabular}
\end{center}
...
\textbf{Model 3} & \textbf{g=7 h=2 w=6} &  \textbf{0.761} & \textbf{20.861} & \textbf{0.830} & \textbf{0.519} & \textbf{532} \\
\hline
\hline
...
\textbf{Model 4} & \textbf{g=7 h=2 w=6} &  \textbf{0.829} & \textbf{11.002} & \textbf{0.861} & \textbf{0.623} & \textbf{1440} \\
\hline
\end{tabular}
\vspace{1mm}
\end{center}
...
\vspace{5mm} 
Below there are some histograms summarizing the results obtained with the Resnet50v2 network. The index data are associated to the hyper-parameters  that gave the best performance in both the Feature Extraction ($g=7, h=2, w=8$) and Fine Tuning ($g=7, h=2, w=6$) approaches. Moreover, for the latter approach we refer to the Model 4 results.\vspace{3mm} \\
\begin{figure}[H]
\centering
...
\item Resnet50v2 (model 4): $g=7, h=2, w=6$
\end{itemize}
\vspace{3mm}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=1.\textwidth]{./img/gui1.png}
        \caption{Web gui, home page}
    \end{minipage}\hfill
    \hspace{1mm}
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[height=0.41\textwidth]{./img/gui2.png}
        \caption{Web gui, select image as query}
    \end{minipage}
\end{figure}
\vspace{3mm}
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.5\textwidth}
        \includegraphics[width=1.1\textwidth]{./img/gui3.png}
    \end{minipage}
    \caption{Web gui, list of results}
\vspace{2mm}
\end{figure}
