...
We performed the evaluation of the sequential search function, based on the validation set, so that we could have some starting data to compare with respect to the performance of the LSH index. We compared the queries to the dataset (training set + distractor), first using the cosine similarity and then the Euclidean distance.\\
...
During this section we extracted the features from the dataset using the pretrained network Resnet50, adopting a features extraction technique and we computed a sequential scan, obtaining the following results:
\begin{center}
...
k = 10 &  mAP euclidian & avg dist & mAP cosine & avg sim & $\#$Items\\
\hline
\textbf{g=5 h=2} & \textbf{0.309} & \textbf{1.216} & \textbf{0.318} & \textbf{0.999}  & \textbf{5701 }\\
g=6 h=2 & 0.303 & 1.227 & 0.308 & 0.999  & 4836 \\
g=7 h=2 & 0.297 & 1.206 & 0.303 & 0.999  & 4872 \\
g=7 h=6 & 0.281 & 1.518 & 0.284 & 0.998  & 1647 \\
g=3 h=4 & 0.294 & 1.376 & 0.293 & 0.998  & 2686 \\
\hline
\end{tabular}
\end{center}
...
During this section we extracted the features from the dataset using the pretrained network Resnet50v2, adopting a features extraction technique and we computed a sequential scan, obtaining the following results:
\begin{center}
\begin{tabular}{|l|cc|cc|c|}
\hline
k = 10 &  mAP euclidian & avg dist & mAP cosine & avg sim & $\#$Items\\
...
k = 10 &  mAP euclidian & avg dist & mAP cosine & avg sim & $\#$Items\\
\hline
g=3 h=2 & 0.369 & 22.772 & 0.375 & 0.619   & 477 \\
\textbf{g=3 h=3} & \textbf{0.415} & \textbf{27.532} & \textbf{0.412} & \textbf{0.490}  & \textbf{79} \\
g=3 h=4 & 0.334 & 29.709 & 0.349 & 0.409   & 25 \\ 
\hline
g=4 h=2 & 0.360 & 22.285 & 0.361 & 0.634   & 430 \\
g=4 h=3 & 0.353 & 27.865 & 0.354 & 0.474   & 71 \\
g=4 h=4 & 0.287 & 29.658 & 0.297 & 0.411   & 27 \\
\hline
g=5 h=2 & 0.311 & 22.456 & 0.309 & 0.627   & 464 \\
g=5 h=3 & 0.368 & 26.175 & 0.373 & 0.527   & 121 \\
g=5 h=4 & 0.263 & 28.903 & 0.278 & 0.432   & 23 \\
\hline
g=6 h=2 & 0.357 & 21.995 & 0.367 & 0.639   & 568 \\
g=6 h=3 & 0.350 & 26.748 & 0.347 & 0.513   & 88 \\
g=6 h=4 & 0.282 & 27.908 & 0.289 & 0.457   & 23 \\
\hline
g=7 h=2 & 0.357 & 22.128 & 0.362 & 0.639   & 685 \\
\textbf{g=7 h=3} & \textbf{0.414} & \textbf{28.075} & \textbf{0.430} & \textbf{0.470}   & \textbf{61} \\
g=7 h=4 & 0.314 & 29.794 & 0.331 & 0.407   & 22 \\
\hline
\end{tabular}
\end{center}
These values are calculated with w=8, from other tests we noted a small decrease on the value, but not so relevant.\\
...
Indeed passing an image as a query, the average euclidian distance between it and its k-nearest neighbor using Resnet50 is about 1.2 while using Resnet50v2 is about 25. Moreover the cosine similarity using Resnet50 is about 0.99 while using Resnet50v2 is about 0.5. \vspace{2mm} \\
...
\hline
 & \textbf{Accuracy} & \textbf{Loss} & \textbf{mAP} euclidian & avg dist & \textbf{mAP} cosine & avg sim & Items\\
\hline
Model 1 & 0.7617 & 0.7307 &  0.312 & 0.493 & 0.309 & 0.987 &32791\\
Model 2 & 0.5935 & 0.9764 &  0.329 & 0.885 & 0.328 & 0.975 &32791\\
Model 3 & 0.6051 & 0.9577 &  0.324 & 1.220 & 0.323 & 0.963 &32791\\
Model 4 & 0.8072 & 0.6162 &  0.326 & 4.413 & 0.326 & 0.943 &32791\\
Model 5 & 0.7675 & 0.6571 &  0.313 & 3.679 & 0.320 & 0.953 &32791\\
\hline
\end{tabular}
\end{center}
...
\begin{tabular}{|l|cc|cc|c|}
\hline
  & \textbf{mAP} euclidian & avg dist & \textbf{mAP} cosine & avg sim & Items\\
\hline
Model 1, g=5 h=3 &  0.314 & 0.574 & 0.318 & 0.982 & 4377\\
Model 2, g=4 h=2 &  0.328 & 1.048 & 0.330 & 0.967 & 4532\\
Model 3, g=4 h=2 &  0.326 & 1.375 & 0.333 & 0.952 & 4507\\
Model 4, g=6 h=3 &  0.316 & 5.647 & 0.328 & 0.912 & 485\\
Model 5, g=4 h=3 &  0.311 & 4.474 & 0.318 & 0.931 & 570\\
\hline
\end{tabular}
\vspace{1mm}
\end{center}
Comparing the performance obtained using different models, we can conclude that computing a sequential scan or using the index, the performance change only in terms of items retrieved.\\
...
\hline
 & \textbf{Accuracy} & \textbf{Loss} & \textbf{mAP} euclidian & avg dist & \textbf{mAP} cosine & avg sim & Items\\
\hline
Model 3 & 0.8493 & 0.4819 &  0.337 & 16.684 & 0.335 & 0.661 &32791\\
Model 4 & 0.8808 & 0.3772 &  0.352 & 8.997 & 0.345 & 0.729 & 32791\\
\hline
\end{tabular}
\end{center}
