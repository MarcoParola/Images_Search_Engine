...
%TODO: non mi torna questo discorso sotto %%
%It is interesting to note that given a certain value of \textit{g} and varying the value of \textit{h}, the best results are given for h=3, because for greater value the mAP always decreases, while for smaller ones (h=2) in some cases the mAP decreases, in others increases, but we also observe a very significant increase in the number of the items to sort.\vspace{2mm} \\
...
In order to better compare the results obtained so far, we reported some other histograms. \\
